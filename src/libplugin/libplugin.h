 ...
 *   int pixi_after_patching(void) -> occurs after all modifications to the rom, will not run if there's errors in between
 ...
 *   Plugin loading order and plugin calling order are UNSPECIFIED, however, the loading order will be the same as the calling order.
...
int for_each_plugin(const std::vector<plugin>& plugins, int (plugins::plugin::*func)(Args...) const, Args... args) {
    for (const auto& p : plugins) {
        if ((p.*func)(args...) != EXIT_SUCCESS) {
            return EXIT_FAILURE;
        };
    }
    return EXIT_SUCCESS;
