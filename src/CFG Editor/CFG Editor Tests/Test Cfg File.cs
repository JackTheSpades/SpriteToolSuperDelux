...
namespace CFG_Editor_Tests
{
    ...
    {
        readonly CfgFile data = new();

        private static void AssertTweakBytes(CfgFile data, byte[] reference)
        {
            Assert.AreEqual(reference[0], data.Type, nameof(CfgFile.Type));
            Assert.AreEqual(reference[1], data.ActLike, nameof(CfgFile.ActLike));

            Assert.AreEqual(reference[2], data.Addr1656, nameof(CfgFile.Addr1656));
            Assert.AreEqual(reference[3], data.Addr1662, nameof(CfgFile.Addr1662));
            Assert.AreEqual(reference[4], data.Addr166E, nameof(CfgFile.Addr166E));
            Assert.AreEqual(reference[5], data.Addr167A, nameof(CfgFile.Addr167A));
            Assert.AreEqual(reference[6], data.Addr1686, nameof(CfgFile.Addr1686));
            Assert.AreEqual(reference[7], data.Addr190F, nameof(CfgFile.Addr190F));

            Assert.AreEqual(reference[8], data.ExProp1, nameof(CfgFile.ExProp1));
            Assert.AreEqual(reference[9], data.ExProp2, nameof(CfgFile.ExProp2));
        }
        ...
        public void TestPixiV1_1Load()
        {
            ...
            Assert.AreEqual(3, data.ByteCount, nameof(CfgFile.ByteCount));
            Assert.AreEqual(4, data.ExByteCount, nameof(CfgFile.ExByteCount));
