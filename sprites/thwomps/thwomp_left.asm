;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Left Horizontal Thwomp, by mikey
...
WAITING:
		...
		BEQ Return0              ;return if Mario is on the right side.
                    
		LDA $0E                  ; get low byte of distance between mario and sprite. (which is negative btw because we're left)
		SEC                      ; so keep in mind, the bigger $0E, the closer we're to the sprite
		SBC #$40                 ; \ To subtract 40 from the value and then not branch (let us pass) if the value is
		PHA                      ; | more or equal to 48 means the value was >= 88 (minus, so 7 1/2 blocks)
		CMP #$48                 ; |
		BCC +                    ; /
		LDA #$01                
		STA !EXPRESSION,x 
+		PLA
		CMP #$60                ; \ if result is less than 60, return.
		BCC Return0             ; / if distance minus 40 is still bigger than 60 it was more than A0 (less than 6 blocks away)
...
.dontInc
		...
		PHA                     ; |
		CLC                     ; |
		ADC #$F6                ; | move sprite x position from center of thwomp
		STA !sprite_x_low,x     ; | to the left side.
		LDA !sprite_x_high,x    ; |
		PHA                     ; |
		ADC #$FF                ; |
		STA !sprite_x_high,x    ; /
